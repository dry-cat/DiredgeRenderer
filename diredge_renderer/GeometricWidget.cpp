...
GeometricWidget::GeometricWidget(GeometricSurfaceFaceDS *newSurface, QWidget *parent)
    : _GEOMETRIC_WIDGET_PARENT_CLASS(parent) { // constructor
    // store pointer to the model
    surface = newSurface;

    // initialise arcballs to 80% of the widget's size
    Ball_Init(&lightBall);
    Ball_Place(&lightBall, qOne, 0.80);
...
// destructor
GeometricWidget::~GeometricWidget() { // destructor
    // nothing yet
} // destructor

// called when OpenGL context is set up
void GeometricWidget::initializeGL() { // GeometricWidget::initializeGL()
    // enable Z-buffering
...
void GeometricWidget::resizeGL(int w, int h) { // GeometricWidget::resizeGL()
    // reset the viewport
    glViewport(0, 0, w, h);
...
void GeometricWidget::paintGL() { // GeometricWidget::paintGL()
    // clear the buffer
...
// mouse-handling
void GeometricWidget::mousePressEvent(QMouseEvent *event) { // GeometricWidget::mousePressEvent()
    ...
    switch (whichButton) { // button switch
    case Qt::RightButton:
        // save the last x, y
        last_x = vNow.x;
        last_y = vNow.y;
...
void GeometricWidget::mouseMoveEvent(QMouseEvent *event) { // GeometricWidget::mouseMoveEvent()
    // find the minimum of height & width
    float size = (width() > height()) ? height() : width();
...
void GeometricWidget::mouseReleaseEvent([[maybe_unused]] QMouseEvent *event) { // GeometricWidget::mouseReleaseEvent()
    // now either translate or rotate object or light
    switch (whichButton) { // button switch
    case Qt::RightButton:
        // just update
